...
impl Executor {
    ...
    async fn process_request(&mut self, tag: u64, content: RequestContent) {
        ...
        match content {
            ...
            RequestContent::ToggleReactionRequest(request) => {
                let result = self.client.toggle_reaction(ctx, request).await;
                if let Err(err) = result {
                    self.send_response(tag, Err(err));
                }
            }
...
mod tests {
    ...
    #[tokio::test]
    async fn test_toggle_reaction_request() {
        // Arrange
        let request = RequestContent::ToggleReactionRequest(ToggleReactionRequest::default());
        ...
        // Assert
        let client = client.as_any().downcast_ref::<ClientMock>().unwrap();
        client.assert_toggle_reaction_called_n(1);

        assert!(output_rx.is_empty())
    }

    #[tokio::test]
    async fn test_toggle_reaction_err() {
        // Arrange
        let request = RequestContent::ToggleReactionRequest(ToggleReactionRequest::default());
        let response = Error {
            r#type: ErrorType::Unknown as i32,
        ...
        let client = ClientMock {
            toggle_reaction_response: Err(response.clone()),
            ..Default::default()
        };
